	...
	<div class="flex flex-1 overflow-hidden">
		<div class="w-48 bg-slate-800/50 border-r border-slate-700 overflow-y-auto">
			<div class="flex justify-between items-center p-2 border-b border-slate-700">
				...
				<button class="text-xs px-2 py-1 rounded bg-cyan-600 hover:bg-cyan-500 text-white cursor-pointer border-none" onclick={newScript}>+ New</button>
			</div>
			{#each scripts as script}
				<button class="w-full text-left px-3 py-2 text-sm text-slate-300 hover:bg-slate-700/50 transition-colors bg-transparent border-none cursor-pointer {selectedScript === script.name ? 'bg-slate-700 text-cyan-300' : ''}" onclick={() => selectScript(script.name)}>📜 {script.name}</button>
			{/each}
