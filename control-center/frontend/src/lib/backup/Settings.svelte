<script lang="ts">
	...
	async function addSession() {
		...
		try { await saveSession(newSessionHost, newSessionPort, newSessionName || newSessionHost); await loadSessions(); newSessionHost = ''; addNotification('success', 'Session saved'); }
...
<div class="p-6 overflow-y-auto h-full box-border">
	...
	<section class="mb-8">
		<h3 class="text-base font-semibold text-slate-200 mb-1">🔌 Wake-on-LAN</h3>
		...
		<div class="flex gap-2 items-end flex-wrap">
			...
			<button class="px-4 py-2 rounded-lg text-sm font-medium bg-cyan-600 hover:bg-cyan-500 text-white disabled:opacity-50 cursor-pointer border-none" onclick={sendWoL} disabled={wolSending}>{wolSending ? 'Sending...' : 'Send Magic Packet'}</button>
		</div>
	</section>

	<section class="mb-8">
		<h3 class="text-base font-semibold text-slate-200 mb-1">💾 Saved Sessions</h3>
		...
		<div class="flex gap-2 items-end flex-wrap mb-3">
			...
			<button class="px-4 py-2 rounded-lg text-sm font-medium bg-cyan-600 hover:bg-cyan-500 text-white cursor-pointer border-none" onclick={addSession}>Save</button>
		</div>
			...
			<div class="space-y-1">
				{#each sessionsList as session (session.id)}
					<div class="flex items-center gap-3 px-3 py-2 rounded-lg bg-slate-800/50 text-sm">
						...
						<span class="text-xs text-slate-600">{new Date(session.last_connected).toLocaleDateString()}</span>
						<button class="text-slate-500 hover:text-red-400 transition-colors bg-transparent border-none cursor-pointer" onclick={() => removeSession(session.id)}>✕</button>
					</div>
				{/each}
			</div>
		{:else}
	...
	<section class="mb-8">
		<h3 class="text-base font-semibold text-slate-200 mb-1">ℹ️ About</h3>
