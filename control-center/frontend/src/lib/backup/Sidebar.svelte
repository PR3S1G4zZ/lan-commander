<script lang="ts">
	...
	import { getOsIcon, formatTimeAgo } from '../utils/format';
	...
	function getStatusColor(agent: AgentInfo): string {
		...
		if (agent.lastSeen && Date.now() - new Date(agent.lastSeen).getTime() < 30000) return 'bg-yellow-500';
		return 'bg-red-500';
	}
</script>
...
<aside class="bg-slate-800 border-r border-slate-700 flex flex-col h-full transition-all duration-200" class:w-64={!$sidebarCollapsed} class:w-12={$sidebarCollapsed}>
	<div class="flex items-center justify-between px-3 py-3 border-b border-slate-700">
		...
		{/if}
		<button class="text-slate-400 hover:text-slate-200 p-1 rounded text-xs bg-transparent border-none cursor-pointer" onclick={() => $sidebarCollapsed = !$sidebarCollapsed}>
			{$sidebarCollapsed ? '▶' : '◀'}
		</button>
	</div>
	...
	{#if !$sidebarCollapsed}
		<div class="p-2">
			<input
				...
				class="w-full bg-slate-700 text-slate-200 rounded px-3 py-1.5 text-sm outline-none border border-slate-600 focus:border-cyan-500 box-border"
			/>
		</div>

		<div class="flex items-center px-3 py-2 text-xs text-slate-400">
			<span class="w-2 h-2 rounded-full inline-block mr-2 flex-shrink-0 bg-green-500"></span>
		...
		<div class="p-3 border-t border-slate-700">
			<button class="w-full flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-medium bg-cyan-600 hover:bg-cyan-500 text-white transition-colors cursor-pointer border-none" onclick={() => showConnectionDialog = true}>
				<span>+</span> Add Agent
			</button>
		</div>
	{:else}
		<div class="flex flex-col items-center gap-2 py-3">
			{#each $agents as agent (agent.id)}
				<button
					...
					title={agent.name}
				>
					<span class="w-2 h-2 rounded-full {agent.connected ? 'bg-green-500' : 'bg-red-500'}"></span>
				</button>
			{/each}
		</div>
	{/if}
